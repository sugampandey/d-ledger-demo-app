    ...
    class VendorsController < Api::BaseController
      ...
      def index
        ...
        if params[:company_id].present?
          ...
          if response.nil?
            api_error('Failed to fetch vendors', :service_unavailable)
          else
            begin
      ...
      def show
        ...
        if params[:id].present? && params[:company_id].present?
          begin
            response = VendorService.get(params[:id], params[:company_id])
            render json: response.body, status: :ok, content_type: 'application/json'
          rescue => e
            Rails.logger.error("Error fetching vendor: #{e.message}")
      ...
      def create
        ...
        valid, error_message = VendorService.valid_csv_file?(params[:file], required_headers)
        ...
        results = VendorService.process_vendor_csv(params[:file], params[:company_id], request.headers)
